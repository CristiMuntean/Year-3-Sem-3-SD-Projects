...
public class EmployeeView extends JFrame implements EmployeeInterface, Observer {
    private final EmployeeController employeeController = EmployeeController.getControllerInstance(this);
    ...
    private void prepareEmployeePanel() {
        ...
        c.fill = GridBagConstraints.HORIZONTAL;
        List<CarJoinedOwner> carsWithOwners = employeeController.getAllCarsWithOwners();

        carsWithOwners = employeeController.sortList(carsWithOwners);
        carsWithOwners = employeeController.filterList(carsWithOwners);

        this.carsTable = employeeController.getCarsWithOwnersTable(carsWithOwners);
        ...
        Arrays.stream(employeeController.getFilterOptions().split(" ")).forEach(element -> {
